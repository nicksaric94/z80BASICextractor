...
// used from mainwindow.cpp (instead of basic_to_ascii.h)
extern int array_index; //used when traversing arrays
extern char buffer[50]; //output buffer for printing numbers;
...
int iterate_over_array(unsigned char* start, int element_length, FILE* output)
{
  ...
  int* dimension_size = (int*) malloc(number_of_dimensions * sizeof(int));
  if (dimension_size == NULL)
  {      
    strcpy(msg,"iterative_over_array(): Neuspesna alokacija.");
    fatal_error(msg);
    return -1;

  }
  int* current_coords = (int*) malloc(number_of_dimensions * sizeof(int));
